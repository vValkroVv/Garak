...
description: Compile LaTeX sources into PDF files inside this diploma report workspace. Use when asked to build, rebuild, or verify a PDF from a `.tex` file under `/Users/valerii.kropotin/НОД/Diploma/reports/16.03.2026`, especially `main.tex`, `main_human.tex`, or `presentation/dualcf_supervisor_presentation_v2.tex`.
---
...
1. Resolve the target `.tex` path relative to the workspace root `/Users/valerii.kropotin/НОД/Diploma/reports/16.03.2026` unless the user gave an absolute path.
...
```bash
python3 .agents/skills/latex-pdf-build/scripts/build_pdf.py presentation/dualcf_supervisor_presentation_v2.tex
```

Build the main report:

```bash
python3 .agents/skills/latex-pdf-build/scripts/build_pdf.py main.tex
```
